...
void guardarCarritoEnArchivo(const CarritoDeCompras& carrito) {
    ...
    for (const auto& producto : carrito.productos) {
        archivo << producto.nombre << " | $" << fixed << setprecision(2) << producto.precio << endl;
    }
    
    archivo << "==========================" << endl;
    archivo << "Subtotal: $" << fixed << setprecision(2) << carrito.subtotal << endl;
    archivo << "Impuestos: $" << fixed << setprecision(2) << carrito.impuestos << endl;
    archivo << "Total: $" << fixed << setprecision(2) << carrito.subtotal + carrito.impuestos << endl;
...
void listarProductosBajoStock() {
    ...
    for (size_t i = 0; i < productos.size(); i++) {
        if (productos[i].stock < 15) {
            ...
            cout << "Precio: $" << fixed << setprecision(2) << productos[i].precio << endl;
...
void listarCarrito(Usuario* usuarioActual) {
    ...
    for (size_t i = 0; i < carritoUsuario->productos.size(); i++) {
        cout << "- " << carritoUsuario->productos[i].nombre << " | $" 
             << fixed << setprecision(2) << carritoUsuario->productos[i].precio << endl;
    }
    
    cout << "Subtotal: $" << fixed << setprecision(2) << carritoUsuario->subtotal << endl;
    cout << "Impuestos: $" << fixed << setprecision(2) << carritoUsuario->impuestos << endl;
    cout << "Total: $" << fixed << setprecision(2) << carritoUsuario->subtotal + carritoUsuario->impuestos << endl;
}

// FUNCIÓN PARCIAL NUEVA: Mostrar los 5 productos con menor stock 
void mostrarProductosMenorStock() {
    ...
    // Crear una copia del vector de productos
    vector<Producto> productosOrdenados = productos;
    
    // Ordenamiento burbuja simple
    for (size_t i = 0; i < productosOrdenados.size() - 1; i++) {
        for (size_t j = 0; j < productosOrdenados.size() - i - 1; j++) {
    ...
    // Mostrar solo los primeros 5 productos (o menos si hay menos de 5)
    int cantidad = productosOrdenados.size() < 5 ? productosOrdenados.size() : 5;
    ...
    for (int i = 0; i < cantidad; i++) {
        cout << productosOrdenados[i].idProducto << " | " 
             ...
             << productosOrdenados[i].stock << endl;
    }
}

// FUNCIÓN PARCIAL NUEVA: Mostrar cantidad de comentarios en una fecha específica
void mostrarComentariosPorFecha() {
    ...
    cout << "Fecha | Cantidad de Comentarios" << endl;
    cout << "--------------------------------" << endl;
    cout << fecha << " | " << contador << endl;
    
    if (contador == 0) {
        cout << "\nNo hay comentarios en esta fecha." << endl;
    } else {
        cout << "\nDetalle de comentarios:" << endl;
        cout << "-----------------------" << endl;
        for (size_t i = 0; i < comentarios.size(); i++) {
            if (comentarios[i].fecha == fecha) {
                cout << "Producto: " << comentarios[i].nombreProducto << endl;
...
// FUNCIÓN PARCIAL NUEVA: Mostrar precio máximo y mínimo de los productos 
void mostrarPrecioMaximoMinimo() {
    ...
    string productoMinimo = productos[0].nombre;
    string productoMaximo = productos[0].nombre;
    int idMinimo = productos[0].idProducto;
    int idMaximo = productos[0].idProducto;
    ...
    for (size_t i = 1; i < productos.size(); i++) {
        if (productos[i].precio < precioMinimo) {
            ...
            productoMinimo = productos[i].nombre;
            idMinimo = productos[i].idProducto;
        }
        if (productos[i].precio > precioMaximo) {
            precioMaximo = productos[i].precio;
            productoMaximo = productos[i].nombre;
            idMaximo = productos[i].idProducto;
        }
    }
    
    cout << "\n=== PRECIO MÁXIMO Y MÍNIMO DE PRODUCTOS ===" << endl;
    cout << "--------------------------------------------" << endl;
    
    cout << "PRECIO MÍNIMO:" << endl;
    cout << "ID: " << idMinimo << endl;
    cout << "Producto: " << productoMinimo << endl;
    cout << "Precio: $" << fixed << setprecision(2) << precioMinimo << endl;
    
    cout << "\nPRECIO MÁXIMO:" << endl;
    cout << "ID: " << idMaximo << endl;
    cout << "Producto: " << productoMaximo << endl;
    cout << "Precio: $" << fixed << setprecision(2) << precioMaximo << endl;
}
...
void menuReportes() {
    ...
    do {
        ...
        cout << "1. Mostrar 5 productos con menor stock" << endl;
        cout << "2. Mostrar comentarios por fecha" << endl;
        cout << "3. Mostrar precio máximo y mínimo" << endl;
